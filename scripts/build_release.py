...
def build(args):
    ...
    run([str(runtime_python.absolute()), str((stage / "manager" / "prepare_portable.py").absolute())],
        cwd=stage)

    print("  Running check_environment.py...")
    run([str(runtime_python.absolute()), str((stage / "manager" / "check_environment.py").absolute())],
        cwd=stage)
